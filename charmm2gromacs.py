	...
	if not presFlag:
		...
		if line.startswith('BOND') or line.startswith('DOUBLE'):
			...
			if firstBond:
				...
				segments = line.split()
				bondNumber = int(len(segments[1:])/2)
	...
	if len(line) > 0:
		...
		if dihedrals and not impropers:
			...
			if line[0] != comment and segments[0] != 'DIHEDRALS':
				...
				if typei == 'X' and typel == 'X':
					...
					if typej != None and typek != None and typel != None:
						bonFile.write(typei+'\t'+
										typej+'\t'+
										typek+'\t'+
										typel+'\t'+
		...
		if impropers and not nonbonded and not cmap:
			...
			if line[0] != comment and segments[0] != 'IMPROPERS':
				typei = segments[0]
				try:typej = segments[1]
				except: pass
				try:typek = segments[2]
				except: pass
				try:typel = segments[3]
				except: pass
				...
				if typej == 'X' and typek == 'X':
					...
					# converstion to kJ
					try:cq = str(float(cq)*2*kcal2kJ) # factor 2 from definition difference
					except:pass
					
					try:bonFile.write(typei+'\t'+
									typej+'\t'+
									typek+'\t'+
									typel+'\t'+
									funcForImpropers+'\t'+
